...
//        //}
            ...
            for (var i=0,len=labels_processed.length; i<len; i+=1) {
                if (labels_processed[i]) {
                    ...
                    if (coords && coords.length > 0) {
                        var x      = (args.object.type == 'bar' ? coords[0] + (coords[2] / 2) : coords[0]);
                        var y      = (args.object.type == 'bar' ? coords[1] + (coords[3] / 2) : coords[1]);
    ...
    //
    RGraph.setShadow = function () // THE ARGS ARGUMENT MUST STAY!
    {
        // 1 Argument
        if (   typeof arguments[0] === 'object'
    ...
    RGraph.stringsToNumbers = function ()
    {
        ...
        if (typeof args.string === 'object' && !RGraph.isNull(args.string)) {
            for (var i=0,len=args.string.length; i<len; i+=1) {
                args.string[i] = parseFloat(args.string[i]);
            }
        }
    ...
    RGraph.drawYAxis = function (obj)
    {
        ...
        //
          ...
          textConfPrefix:   'yaxisLabels',
                ...
                obj.maxLabelLength = Math.max(obj.maxLabelLength, obj.context.measureText(String(properties.yaxisLabelsSpecific[i])).width * 2);
            }
        }
...
// End module pattern
    ...
    window.$d = function (m)
    {
        ...
        if (!this.rgraph_debug_textarea) {
                ...
                this.rgraph_debug_textarea.style.position = 'absolute';
